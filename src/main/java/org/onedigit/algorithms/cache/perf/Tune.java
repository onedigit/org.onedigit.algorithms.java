...
public class Tune
{
	public static void main(String[] args)
	{
		...
		/*
		try {
			times = ((new Integer(args[0])).intValue());
			k = ((new Integer(args[1])).intValue());
			STRIDE = ((new Integer(args[2])).intValue());
			NUM_WAYS = ((new Integer(args[3])).intValue());
			thrd = ((new Integer(args[4])).intValue());
			isVerbose = ((new Boolean(args[5])).booleanValue());
			debug = ((new Boolean(args[6])).booleanValue());
		} catch (Exception e) {
		...
		*/
		...
		while (counter < times) {
			...
			if (first) {
				...
				TOTAL_ACCESS = 
						(int) ((targetTime * (double) TOTAL_ACCESS / (NUM_WAYS * STRIDE)) / 
								((double) (stop_time - start_time))) * (NUM_WAYS * STRIDE);
				if (isVerbose) {
		...
		for (i = 0; i < arr_items - NUM_WAYS * STRIDE; i += NUM_WAYS * STRIDE) {
			for (j = i; j < i + NUM_WAYS * STRIDE; j += STRIDE) {
				va[j] = j + NUM_WAYS * STRIDE;
			}
		}
		 ...
		 * TOTAL_ACCESS / (double) (NUM_WAYS * STRIDE)) / ((double) ((stop_time
		 * - start_time))); System.out.println("MAPS " + resl1 + " @ " + L1 * 3
		 * / 4 + "KB");
		 */
		/*-------- end single mb test--------*/
		 ...
		 * TOTAL_ACCESS / (double) (NUM_WAYS * STRIDE)) / ((double) ((stop_time
		 * - start_time))); System.out.println("MAPS " + resl2 + " @ " + L2 * 3
		 * / 4 + "KB");
		 */
		/*-------- end single mb test--------*/
